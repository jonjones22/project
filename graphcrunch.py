    ...
    if sys.argv[1] == 'f1' or sys.argv[1] == 'xf1':
        ...
        for vname in sys.argv[3:]:
            ...
            if miny < 0:
                ...
                miny = min(miny, v)
            xlbls[tid].append(vname.replace('SUM_dealCards_thread_','T').replace('_process_','P'))
            csvhdr = csvhdr + "," + vname
        ...
        ax.set_title("time-to-deal by pairs of partner threads, "
        ...
        for fname in sys.argv[3:]:
            ...
            if miny < 0:
                ...
                miny = min(miny, v)
            xlbls.append(fname[:-7].replace('SUM_dealCards_thread_','T').replace('_process_','P'))
            csvhdr = csvhdr + "," + fname
            csvvalues = csvvalues + "," + str(v)
